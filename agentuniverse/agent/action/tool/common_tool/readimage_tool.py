...
def detect_text_regions(image, east_model: str='frozen_east_text_detection.pb', min_confidence: float=0.5, width: int=320, height: int=320):
    """
...
def ocr_on_regions(regions, lang: str='chi_sim+eng'):
    """
...
def save_text_to_file(text, output_file: str='extracted_text.txt'):
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"Text content saved to {output_file}")

def extract_text_from_image(image_path, use_east: bool=True, lang: str='chi_sim+eng'):
    """
